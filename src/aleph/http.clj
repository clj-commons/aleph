...
(defn start-server
  "Starts an HTTP server using the provided Ring `handler`.  Returns a server
   ...
   | `thread-pool-name`                | Name of the thread pool to use for the Netty EventLoopGroup. Defaults to \\\"aleph-server-pool\\\". |\r
...
(defn connection-pool
  "Returns a connection pool which can be used as an argument in `request`.
   ...
   | `thread-pool-name`        | Name of the thread pool to use for the Netty EventLoopGroup. If provided, multiple pools with the same name will share the same EventLoopGroup.
